...
public class DevNexusAuthenticator extends AbstractAccountAuthenticator {
    ...
    private final Class<?> authenticationActivityClass;
    public static final String ACCOUNT_TYPE = "org.devnexus.Account";

    GooglePlusAuthenticationModule module;
    ...
    public DevNexusAuthenticator(Context context) {
        ...
        String className = ai.metaData.getString("org.devnexus.AuthenticationActivity");
        if (className == null) {
            throw new IllegalStateException("You must define a property : org.devnexus.AuthenticationActivity");
        }
        
        authenticationActivityClass = Class.forName(className);
        
            module = new GooglePlusAuthenticationModule(new URL(Constants.DEVNEXUS_URL), config, context);
        } catch (MalformedURLException | PackageManager.NameNotFoundException | ClassNotFoundException ex) {
            Logger.getLogger(CookieAuthenticator.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public Bundle addAccount(AccountAuthenticatorResponse response, String accountType, String authTokenType, String[] requiredFeatures, Bundle options) throws NetworkErrorException {
        ...
        toReturn.putParcelable(AccountManager.KEY_INTENT, new Intent(context, authenticationActivityClass).putExtra(AccountManager.KEY_ACCOUNT_AUTHENTICATOR_RESPONSE, response));
